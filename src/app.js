...
function displayBookmarks(bookmarkNodes) {
    ...
    list.className = "list-group d-flex flex-column gap-3 h-auto";
    for (let node of bookmarkNodes) {
        // validate favorites data before passing to displayBookmark
        if (isValidBookmark(node)) {
            let listItem = displayBookmark(node);
            if (listItem) {
                list.appendChild(listItem);
            }
        } else {
            console.error("Invalid bookmark data found: ", node);
        }
        if (node.children) {
            let childList = displayBookmarks(node.children);
